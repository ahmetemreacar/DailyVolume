"""1dk acilis | 2026 | TP %3 SL %3 | 3x/5x x DIP+KIRILIM / +orta karsilastirma."""
...
def main():
    ...
    print(f"1dk tarama | {len(hisseler)} hisse | {BASLANGIC}+ ...", file=sys.stderr)
    tum, bar_veri = topla(hisseler, Cfg(), ACILIS_SON)
    tum = [s for s in tum if dt.date.fromisoformat(s["tarih"]) >= BASLANGIC]

    if not tum:
        print("Sinyal yok (1dk veri kapsami sinirli olabilir).")
        return

    tarihler = sorted({s["tarih"] for s in tum})
    print("=" * 80)
    print(f"1DK ACILIS | TP %3 SL %3 | compound | slip yok | {Cfg.evren}")
    print(f"Veri araligi: {tarihler[0]} -> {tarihler[-1]} | toplam sinyal (tum gruplar): {len(tum)}")
    print("NOT: 1dk max 5000 bar -> 2026 tamami degil, mevcut cache araligi kullanildi.")
    print("=" * 80)
    ...
    odf.to_csv("test_kat_orta_1dk_ozet.csv", index=False)
    print("\n" + "=" * 80)
    print(odf.to_string(index=False))
    print("\n-> test_kat_orta_1dk_ozet.csv")
